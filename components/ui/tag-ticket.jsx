...
const TagTicket = ({tagName, removeClickHandle}) => { 
    return (
        ...
        border-primary focus-visible:ring-1 focus-visible:ring-ring focus-visible:ring-offset-1 ">
                <div className="flex w-2/4 flex-col text-white text-xs font-semibold font-['Inter']">{tagName}</div>
                <Button className="flex w-1/4 flex-col" variant="ghost" size="h-5" onClick={e => removeClickHandle(tagName)}>
                    <X className="h-6"/>
                </Button>
        </div>
    )
